...
const VoiceIntakeForm = ({navigation, route}) => {
  ...
  const StartChart = async () => {
    ...
    const response = await fetch(
      Global.BASE_URL +
        `createCall&userId=${userId}&astrologerId=${AstrologerId}&roomId=${RoomId}userId=${userId}&astrologerId=${AstrologerId}&roomId=${RoomId}&status=created&name=${Name}&dob=${date}&gender=${gender}&tob=${TOB}&pob=${POB}&lat=${Latitude}&lon=${Longitude}`,
    );
    const data = await response.json();
    console.log(data);
    if (data.response.status == 1) {
      const collectionset = firestore().collection('call');
    ...
    <View>
      <ScrollView style={{}} showsVerticalScrollIndicator={false}>
        ...
        <TouchableOpacity
          ...
          <Text style={styles.createProfileButtonText}>Start Call</Text>
        </TouchableOpacity>
      </ScrollView>
